...
LastEditTime: 2021-06-02 02:39:32
LastEditors: yanxinhao
Description: 
Date: 2021-06-02 02:39:28
FilePath: /3DFMM/core/morphable_model/Flame/FLAME.py
...
class FLAME(nn.Module):
    ...
    def __init__(self, config, image_size=None, device="cuda"):
        ...
        shapedirs = torch.cat(
            [
                shapedirs[:, :, : config.shape_params],
                shapedirs[:, :, 300 : 300 + config.expression_params],
            ],
            2,
        )
    ...
    def initialize_params(self, batch_size=1):
        self.shape = nn.Parameter(
            torch.zeros(batch_size, self.config.shape_params).float().to(self.device)
        )
        self.tex = nn.Parameter(
            torch.zeros(batch_size, self.config.tex_params).float().to(self.device)
        )
        self.exp = nn.Parameter(
            torch.zeros(batch_size, self.config.expression_params)
            .float()
            .to(self.device)
        )
        self.pose = nn.Parameter(
            torch.zeros(batch_size, self.config.pose_params).float().to(self.device)
        )
        self.lights = nn.Parameter(
            torch.zeros(batch_size, 9, 3).float().to(self.device)
        )
        if self.is_perspective:
            ...
            self.cam_t = nn.Parameter(
                torch.zeros(batch_size, 2).float().to(self.device)
            )
    ...
    def setup_camera(self, camera=None, camera_path=None, is_perspective=True):
        ...
        if self.is_perspective:
            ...
            self.cam = Camera(
                camera_path=camera_path,
                is_perspective=is_perspective,
                scale=nn.Parameter(0.7 * torch.ones(1).float().to(self.device)),
            )
    ...
    def load_shape(self, shape_path):
        ...
        self.shape = nn.Parameter(torch.tensor(shape).unsqueeze(0).to(self.device))

    def _setup_renderer(self, image_size):
        mesh_file = "./Data/morphable_model/FLAME/head_template_mesh.obj"
        self.render = Renderer(
            (image_size[1], image_size[0]), obj_filename=mesh_file
        ).to(self.device)
    ...
    def forward(
        ...
        """
        Input:
            scale : to make scatter's varition be in [-1,1]
        ...
        # scale the vertices to make scatter's varition be in [-1,1]
        vertices *= scale
    ...
    def optimize_by_landmark(
        ...
        e_opt = torch.optim.Adam(
            [self.shape, self.exp, self.pose, self.tex, self.cam_t, self.lights]
            + self.cam.get_params(),
            lr=self.config.e_lr,
        ...
        for k in range(max_iterations):
            ...
            losses["landmark"] = (
                l2_distance(landmarks2d[:, 17:, :2], gt_landmark[:, 17:, :2])
                * self.config.w_lmks
            )
            ...
            if k % 10 == 0:
                ...
                visind = range(bz)  # [0]
                grids["images"] = (
                    torchvision.utils.make_grid(images[visind]).detach().cpu()
                )
        ...
        for k in range(200, 1000):
            ...
            losses["landmark"] = (
                l2_distance(landmarks2d[:, :, :2], gt_landmark[:, :, :2])
                * self.config.w_lmks
            )
            losses["shape_reg"] = (
                torch.sum(self.shape ** 2) / 2
            ) * self.config.w_shape_reg  # *1e-4
            losses["expression_reg"] = (
                torch.sum(self.exp ** 2) / 2
            ) * self.config.w_expr_reg  # *1e-4
            losses["pose_reg"] = (
                torch.sum(self.pose ** 2) / 2
            ) * self.config.w_pose_reg

            # render
            if photometric:
                albedos = self.flametex(self.tex) / 255.0
                ops = self.render(vertices, trans_vertices, albedos, self.lights)
                predicted_images = ops["images"]
                losses["photometric_texture"] = (
                    image_masks * (ops["images"] - images).abs()
                ).mean() * self.config.w_pho
    ...
    def identity_fitting(self, dataloader, epochs=15, savefolder="./"):
        ...
        for i in range(epochs):
            ...
            results = {"shape": self.shape[0].cpu().detach().numpy().tolist()}
    ...
    def _fit_landmarks(
        ...
        # init optimizer
        e_opt = torch.optim.Adam(
            opt_params, lr=self.config.e_lr, weight_decay=self.config.e_wd
        )
        e_opt_rigid = torch.optim.Adam(
            opt_rigid_params, lr=self.config.e_lr, weight_decay=self.config.e_wd
        )
        gt_landmark = landmarks
        ...
        if log:
            ...
            grids["shape"] = (
                torchvision.utils.make_grid(
                    F.interpolate(
                        shape_images[visind], (self.image_size[1], self.image_size[0])
                    )
                )
                .detach()
                .float()
            ...
            check_mkdir("./tmp")
            cv2.imwrite(
                "{}/batch_{}.jpg".format("./tmp", self.batch_idx),
                grid_image,
            )

    def save_obj(self, save_path="./Results/test.obj", **params):
        """save the face model which is in world coord

        Args:
            save_path (str, optional): [description]. Defaults to "./Results/test.obj".
        """
        vertices, _, _ = self.forward(
            shape_params=params["shape"].to(self.device),
            expression_params=params["expression"].to(self.device),
            pose_params=params["pose"].to(self.device),
        )
        self.render.save_obj(filename=save_path, vertices=vertices[0])

    def render_shape(self, **params):
        # must setup camera before calling render_shape
        vertices, _, _ = self.forward(
            shape_params=params["shape"].to(self.device),
            expression_params=params["expression"].to(self.device),
            pose_params=params["pose"].to(self.device),
        )
        cam_t = params["cam_t"].to(self.device)
        trans_vertices = self.project_fun(vertices, self.cam, cam_t)
        shape_images = self.render.render_shape(
            vertices,
            trans_vertices,
        )
        # rgb2bgr
        shape_images = (
            shape_images.detach().cpu().numpy().transpose(0, 2, 3, 1).copy() * 255
        )[:, :, :, [2, 1, 0]]
        return shape_images
