...
LastEditTime: 2021-04-23 06:43:59
LastEditors: yanxinhao
Description: 
reference : https://github.com/YadiraF/face3d/blob/master/face3d/morphable_model/morphabel_model.py
'''
...
class BFM(object):
    ...
    def generate_vertices(self, shape_para, exp_para):
        ...
        vertices = np.reshape(
            vertices, [int(3), int(len(vertices) / 3)], 'F').T
    ...
    def generate_colors(self, tex_para):
        ...
        colors = np.reshape(
            colors, [int(3), int(len(colors) / 3)], 'F').T / 255.
    ...
    def transform_3ddfa(self, vertices, s, angles, t3d):
    ...
    #             x, X_ind, self.model, n_sp=self.n_shape_para, n_ep=self.n_exp_para, max_iter=max_iter)
