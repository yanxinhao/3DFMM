...
from .utils import dict2obj, face_vertices, vertex_normals, save_obj
...
class Renderer(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256):
        ...
        constant_factor = torch.tensor(
            [1 / np.sqrt(4 * pi), ((2 * pi) / 3) * (np.sqrt(3 / (4 * pi))), ((2 * pi) / 3) * (np.sqrt(3 / (4 * pi))),
             ((2 * pi) / 3) * (np.sqrt(3 / (4 * pi))), (pi / 4) *
             (3) * (np.sqrt(5 / (12 * pi))),
             (pi / 4) * (3) * (np.sqrt(5 / (12 * pi))),
             (pi / 4) * (3) * (np.sqrt(5 / (12 * pi))), (pi / 4) *
             (3 / 2) * (np.sqrt(5 / (12 * pi))),
    ...
    def world2uv(self, vertices):
        ...
        batch_size = vertices.shape[0]
        face_vtxs = face_vertices(
            vertices, self.faces.expand(batch_size, -1, -1)).clone().detach()
        uv_vertices = self.uv_rasterizer(self.uvcoords.expand(batch_size, -1, -1),
                                         self.uvfaces.expand(batch_size, -1, -1), face_vtxs)[:, :3]

        return uv_vertices

    def save_obj(self, filename, vertices, textures=None):
        '''
        vertices: [nv, 3], tensor
        texture: [3, h, w], tensor
        '''
        save_obj(filename, vertices, self.faces[0], textures=textures, uvcoords=self.raw_uvcoords[0],
